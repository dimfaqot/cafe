...
<button class="btn btn-sm btn-primary copy_table">PENGELUARAN</button>
<button class="btn btn-sm btn-success copy_table">TRANSAKSI</button>
<button class="btn btn-sm btn-secondary copy_table">HUTANG</button>
...
<script>
    ...
    $(document).on('click', '.copy_table', function(e) {
        e.preventDefault();
        let tabel = $(this).text().toLocaleLowerCase();
        loading("close");
        post("settings/copy_table", {
            tabel
        }).then(res => {
            if (res.status == "200") {
