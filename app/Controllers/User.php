...
class User extends BaseController
{
    ...
    public function add()
    {
        ...
        if (db(menu()['tabel'])->where('nama', $input['nama'])->countAllResults() > 0) {
            gagal(base_url(menu()['controller']), 'Nama existed');
        }
        // Cek duplikat
        if (db(menu()['tabel'])->where('wa', $input['wa'])->countAllResults() > 0) {
            gagal(base_url(menu()['controller']), 'W.a existed');
        }
    ...
    public function edit()
    {
        ...
        if ((db(menu()['tabel'])->whereNotIn('id', [$id]))->where("wa", $q['wa'])->get()->getRowArray()) {
            gagal(base_url(menu()['controller']), "W.a existed");
        }
